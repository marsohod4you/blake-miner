...
static struct usb_endpoints ftdi2232h_eps[] = {
	//{ LIBUSB_TRANSFER_TYPE_BULK,	512,	EPI(1), 0 },
	//{ LIBUSB_TRANSFER_TYPE_BULK,	512,	EPO(2), 0 },
	{ LIBUSB_TRANSFER_TYPE_BULK,	512,	EPI(3), 0 },
	{ LIBUSB_TRANSFER_TYPE_BULK,	512,	EPO(4), 0 }
};
...
static int _usb_init(struct cgpu_info *cgpu, struct libusb_device *dev, struct usb_find_devices *found)
{
	...
	applog(LOG_ERR,"USB init, found->epcount %d",found->epcount);
	...
	applog(LOG_ERR,"USB init, come to claim interface %d err %d",found->interface,err);
	if (err) {
		switch(err) {
			case LIBUSB_ERROR_BUSY:
				applog(LOG_WARNING,
